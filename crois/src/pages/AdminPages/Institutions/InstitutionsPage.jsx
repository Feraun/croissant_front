...
import EditInstitutionModal from "./EditInstitutionsModal";
  ...
  const actions = (record, loadData) => (
    <Space>
      <Button
        type="link"
        onClick={async () => {
          const city = await adminInstitutionsService.getInstitutionById(record.id);
      ...
      <Button
        ...
        type="link"
        onClick={async () => {
          await adminInstitutionsService.deleteInstitution(record.id);
          loadData();
        }}
      >
        Удалить
  ...
  return (
    <>
      ...
      <TableComponent
        key={tableKey}
        columns={columns}
        fetchData={adminInstitutionsService.searchInstitutions}
        actions={actions}
